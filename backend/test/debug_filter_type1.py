...
import sys
import os
# 添加项目根目录（包含shared）和backend目录到搜索路径
...
from stock_filter.stock_analyzer import StockAnalyzer
...
# 2. 检查是否为主板股票
print("【步骤2: 主板股票检查】")
is_main_board = stock_analyzer.check_is_main_board(symbol)
print(f"  主板股票: {'✓ 是' if is_main_board else '✗ 否'}")

# 3. 检查是否为10cm股票
print("【步骤3: 10cm股票检查】")
is_10cm = stock_analyzer.check_is_10cm(symbol)
...
performance = stock_analyzer.check_performance(symbol, trade_date, params)
